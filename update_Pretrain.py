...
valid_taxonomies = {
    key: "Knowledge-enhanced" if "knowledge" in key else value
    for key, value in {
        "global-local": "Multi-scale",
...
multi_scale_ar = []
data_efficient_ar = []
knowledge_enhanced_ar = []
others_ar = []

# 读取CSV文件
with open('./Pre-training-unpro.csv', 'r', encoding='utf-8') as csvfile:
    ...
    for row in reader:
        if row['Year'].startswith('2024'):
            ...
            # 检查taxonomy是否在有效集合中
            matched = False
            for key in valid_taxonomies.keys():
                if taxonomy.startswith(key):
                    matched = True
                    new_taxonomy = valid_taxonomies[key]
                    PaperName = row['Name']
                    ...
                    if Publications.lower() == 'arxiv':
                        if new_taxonomy == 'Multi-scale':
                            multi_scale_ar.append(info)
                        elif new_taxonomy == 'Data-efficient':
                            data_efficient_ar.append(info)
                        elif new_taxonomy == 'Knowledge-enhanced':
                            knowledge_enhanced_ar.append(info)
                        else:
                            others_ar.append(info)
                    else:
                        if new_taxonomy == 'Multi-scale':
                            multi_scale.append(info)
                        elif new_taxonomy == 'Data-efficient':
                            data_efficient.append(info)
                        elif new_taxonomy == 'Knowledge-enhanced':
                            knowledge_enhanced.append(info)
                        else:
                            others.append(info)
                    break
...
# 将ignore_info写入Pre-trainingIgnore.csv
# with open('Pre-trainingIgnore.csv', 'w', encoding='utf-8', newline='') as csvfile:
#     writer = csv.DictWriter(csvfile, fieldnames=reader.fieldnames)
#     writer.writeheader()
#     writer.writerows(ignore_info)

# 合并arxiv和非arxiv列表
multi_scale.extend(multi_scale_ar)
data_efficient.extend(data_efficient_ar)
knowledge_enhanced.extend(knowledge_enhanced_ar)
others.extend(others_ar)
print("...")
# 将列表内容写入preTraining00.txt文件
with open('./preTraining.txt', 'w', encoding='utf-8') as txtfile:
    def write_section(title, content):
        txtfile.write(f"# {title}\n\n")
        txtfile.write("\n".join(content))
        txtfile.write("\n\n")
    
    write_section("**Multi-scale**", multi_scale)
    write_section("**Data-efficient**", data_efficient)
    write_section("**Knowledge-enhanced**", knowledge_enhanced)
    write_section("**Others**", others)
